...
async def get_chat_id_topic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if topic_id is None:
        ...
        await update.message.reply_text(
            f"Id della chat: {chat_id}, id del topic: {topic_id}"
        )
...
async def deep_linked_level_1(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        await update.message.reply_text(
            f"Sei autenticat*. La tua email è {user_mail}@eagletrt.it!"
        )
    else:
        await update.message.reply_text(
            "Attenzione: utilizza il comando /ore altrimenti contatta lo staff IT"
        )
        return
